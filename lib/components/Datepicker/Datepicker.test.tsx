...
describe('Datepicker', () => {
  ...
  it('renders correctly', () => {
    ...
    const { container } = render(
      <Datepicker label={label} placeholder={placeholder} value={value} onValueChange={onValueChange} />,
    );
  ...
  it('renders the correct label', () => {
    render(<Datepicker label={label} placeholder={placeholder} value={value} onValueChange={onValueChange} />);
  ...
  it('calls onValueChange when a new date is typed into the input', async () => {
    render(<Datepicker label={label} placeholder={placeholder} onValueChange={onValueChange} />);
    ...
    expect(onValueChange).toHaveBeenCalledWith(
      expect.objectContaining({
        value: [expect.objectContaining({ year: 2024, month: 6, day: 1 })],
        valueAsString: ['2024-06-01'],
      }),
    );
  });
});
