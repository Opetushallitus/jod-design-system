  ...
  return (
    <div className={`${className ? className : ''} flex flex-col items-center justify-center px-1`.trim()}>
      <Component
        ...
        className={`flex size-[72px] rounded-full border border-transparent no-underline hover:ring hover:ring-purple-500 ${selected ? 'bg-[#697077]' : 'bg-[#f5f5f5]'}`}
      >
        {/** TODO: Need real icons and alignment and styles to be adjusted then */}
        <span
          aria-hidden={true}
          className={`flex flex-1 select-none justify-center pt-1 text-5xl ${selected ? 'text-[#ffffff]' : 'text-[#4d5358]'}`}
        >
          &#9776;
        </span>
      </Component>
